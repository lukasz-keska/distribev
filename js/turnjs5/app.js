...
var FlipbookView = SingleView.extend({
  ...
  _getPageElement: function(pageNumber) {
      ...
      console.log('1getPageElement',pageNumber)
      
    var $el = $('<div />'),
      settings = window.FlipbookSettings,
      //imgSrc = settings.pageFolder+'/'+pageNumber+'/'+pageNumber+'.jpg',
      imgSrc = $('.pages_list>img:eq('+(pageNumber-1)+')').attr('src'),
      $img = $('<img />', { class:'page_nb_'+pageNumber, width: '100%', height: '100%', css: {display: 'none'} });
...
var MiniaturesView = SingleView.extend({
  ...
  _refreshPictureEvent: function(event, pageNumber, $pageElement) {
    ...
    if (!$img[0]) {
      $pageElement.html('<img class="page_nb_'+pageNumber+'" width="100%" height="100%" src="'+imgSrc+'"/>');
    } else {
     $img.attr('src', imgSrc);
    }
  },
